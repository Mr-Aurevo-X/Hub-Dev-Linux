...
def _group_label(group_id: str) -> str:
    return i18n.t(f"nav_group_{group_id}")


def _page_label(page: NavPage) -> str:
    return i18n.t(page.label_key or page.key)


def nav_pages() -> tuple[tuple[str, str], ...]:
    return tuple((page.key, page.label_key or page.key) for group in nav_groups() for page in group.pages)


def page_titles() -> dict[str, str]:
    return {page.key: _page_label(page) for group in nav_groups() for page in group.pages}
...
class NavSidebar:
    ...
    def __init__(
        ...
        *,
        ...
        self._page_meta: dict[str, NavPage] = {}
    ...
    def _on_row_selected(self, _listbox: Gtk.ListBox, row: Gtk.ListBoxRow | None) -> None:
        if self._selecting or row is None or not row.get_sensitive():
            return
        key = row.get_name()
        if key:
    ...
    def _build(self) -> Gtk.Widget:
        ...
        for group in nav_groups():
            ...
            for page in group.pages:
                ...
                row.set_sensitive(page.enabled)
                ...
                text = Gtk.Label(label=_page_label(page), xalign=0)
                text.set_hexpand(True)
                self._page_labels[page.key] = text
                self._page_meta[page.key] = page
                box.append(icon)
                box.append(text)
                row.set_child(box)
                listbox.append(row)
    ...
    def relabel(self) -> None:
        ...
        for key, label in self._page_labels.items():
            page = self._page_meta.get(key)
            label.set_text(_page_label(page) if page is not None else i18n.t(key))
